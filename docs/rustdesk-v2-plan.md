...
(история) и [rustdesk-deployment.md](rustdesk-deployment.md) (раскатка).
...
| 4.5 Worker | **сделано** |
| 5. Фронтенд | **сделано** — вкладки «Устройства/Админы/Подключения», чип готовности, модалка команды развёртывания, панель учёток |
| 6. Тесты бэкенда | **сделано** (`test_remote_access_service.py`, `test_remote_access_console.py`, `test_rustdesk_client.py`) |
| 6. Тесты фронтенда | **сделано** — 29/29 файлов и 101/101 тест зелёные (`npx vitest run`) |
| 7. Выкат на прод | **сделано и живо проверено** — коммит `0d09a9a` на сервере, три реальных бага API консоли найдены и исправлены (см. §9) |
| 8. Windows-редакция + AppLocker | **сделано** (миграция `e2f3a4b5c6d7`) — см. §10 |

---

## 10. Windows-редакция машины и честность блокировки AppLocker

`desired_block_outgoing` (включён по умолчанию) реализован через AppLocker —
но AppLocker до сентября 2022 (KB5024351) работал только на Enterprise/Education,
и **никогда** не работает на Windows Home (нет `AppIDSvc`). До этой доработки
InfraScope молча предполагал, что блокировка применилась везде.

Отдельного канала для опроса редакции ОС по всему парку нет и специально не
заводился — WinRM закрыт на большинстве машин, а новый remote-query surface
это ровно то, чего избегает вся архитектура (см. §1, lateral movement).
Вместо этого: **скрипт раскатки сам читает свою редакцию** (уже работает от
SYSTEM локально) и передаёт её в каждом отчёте:

* `HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\EditionID` — не зависит
  от локали (`Professional`, `Enterprise`, `Core` = Home), в отличие от
  `Caption` (`"Windows 10 Домашняя"` на русской локали).
* Классификация — на бэкенде (`service.classify_applocker_support`), не в
  PowerShell: один тестируемый источник истины. `Core*` → `False`, всё
  остальное → `True`.
* Честная оговорка: очень старый непропатченный Windows 10 Pro (до
  KB5024351) технически не подпадёт под эту проверку — редакция сама по себе
  этого не покажет. На парке 2026 года, далеко за пределами mainstream
  support Windows 10, это не ожидается, но стоит иметь в виду.

**Покрытие** — только машины, хоть раз прогнавшие скрипт раскатки (обычный
или повторный). Чтобы получить данные по **всему** парку сразу, включая уже
настроенные вручную машины: скрипт идемпотентен, поэтому та же команда «Команда
развёртывания», запущенная на всех устройствах, безопасно прогоняется
даже там, где всё уже настроено — за секунды, без переустановки, просто
досылает актуальную редакцию.

**В UI:** значок «Home» рядом с чипом готовности (только когда несовпадение
подтверждено — не когда редакция ещё неизвестна), баннер со счётчиком над
таблицей устройств, разбор в подсказке чипа.

**Передеплой с сохранением настроек.** Кнопка «Развернуть» и так реюзабельна
для любого устройства — скрипт на каждом прогоне перезаписывает `RustDesk2.toml`
целиком заново из текущего желаемого конфига (id, пароль, hidden/block_outgoing/
unattended), не только то, что изменилось. Что добавлено сейчас:

* Метка кнопки — «Передеплоить» вместо «Развернуть», как только устройство хоть
  раз успешно сконфигурировано (`readiness` = ready/installed_offline/stale).
* Новое состояние `deploy_state = "stale"` — выставляется автоматически при
  ротации пароля или ручном изменении ID/пароля/hidden/block_outgoing/unattended
  на уже настроенном устройстве, чтобы не создавать иллюзию, что новый пароль
  уже применился на машине. Снимается следующим успешным отчётом от скрипта.
...
отвергнутым: он неотличим от lateral movement, и защитное ПО на парке его блокирует.
...
1. **GPO computer startup script** — работает по расписанию и доживается до
   включения машины.
2. **`schtasks /S <host> /RU SYSTEM`** с админской рабочей станции — для точечной
...
пересобирать оффлайн-пакет не нужно. Это главное отличие от текущей схемы, где
...
Пароль всё равно попадает на машину — иначе клиент не настроить; в схеме с оффлайн-пакетом
...
3. Если RustDesk уже нужной версии и конфиг совпадает — рапортует `configured`
   и выходит (идемпотентность, чтобы задачу можно было гонять по расписанию).
...
Существующий `rustdesk-ksc/configure.ps1` остаётся для варианта «оффлайн-пакет без
...
GET    /remote-access/deploy/command             одна строка для GPO/schtasks
...
Старый `GET /devices/{id}/package` остаётся — он нужен для оффлайн-пакета.
...
3. **`DeployCommandModal.tsx`** — общая модалка с командой из
   ...
   (GPO/schtasks); отдельно показывает предупреждение, если
...
5. **`RemoteAccessButtons.tsx`** — чип готовности вместо пары иконок; кнопка
   «Развернуть» вместо «Пакет» как основное действие; оффлайн-пакет
   остался как второстепенная иконка-кнопка (нужен для машин без сети до
   InfraScope, см. §6 в `rustdesk-deployment.md`).
...
7. GPO или push с рабочей станции админа; для касс расписание — вне бизнес-часов.
...
| Отвергнуто | Почему |
|---|---|
| Push с Linux-сервера (`impacket`/`wmiexec`) | неотличим от lateral movement, блокируется защитным ПО на парке |
| Свой Windows-агент InfraScope | нужен стабильный доменный хост и учётка; push с рабочей станции админа уже делает это |
